...
Doc Agent 作为独立的 LangGraph 子图运行，节点与流转如下：
...
- **assess 节点：** 完备度判断——确定性条件优先（结果≥5 AND req_ids 命中→收敛），不满足时 LLM 兜底
- **条件边：** assess → 收敛则 END，不收敛则携带新线索回到 search（最多 3 轮）
...
**重放流程：** 对每条 query，从日志读取 BM25 Top-20（含 rank）和向量 Top-20（含 rank）。对每组候选权重 `(w_bm25, w_vector)`，为每个候选 chunk 计算加权 RRF 分数——公式为 `w_bm25/(60+bm25_rank) + w_vector/(60+vector_rank)`。未进入某个检索器 Top-20 的 chunk，该检索器 rank 视为 100。按加权 RRF 降序取 Top-10，对照标注集计算 NDCG@10。
...
Doc Agent 运行时根据 Supervisor 下发的实体自动选择对应权重组：`req_ids` 非空 → precise 模式（BM25 主导）；`module` 非空 → hybrid 模式（等权）；否则 → semantic 模式（向量主导）。
