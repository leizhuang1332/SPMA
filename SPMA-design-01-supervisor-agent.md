# Design: Supervisor Agent 设计（查询理解层）

> 所属项目：[SPMA 全局概览](SPMA-design-00-global-overview.md)
> 模块职责：理解用户意图、抽取关键实体、改写查询以桥接词汇鸿沟，将结构化的检索参数下发给各 Worker
...
| `module` | 仓库路由 + 关键词构造——确定目标仓库 + 中文术语翻译为英文代码标识符 | "用户登录"→限定 `auth-service` 仓库 + grep `["login","auth","oauth"]` |
...
**注意 SQL Worker 的陷阱：** 去掉实体后 Recall 可能不降——语义搜索总能返回 N 个相关的表，里面很可能包含正确的那个。但 LLM 从 N 个候选表中选对一个的准确率，跟只有 1 个确定表完全不同。这是最危险的退化模式——指标看起来 OK，用户体验已经崩了。
...
### 4.2 方案 1：标准化（Normalization）— 始终开启
...
### 4.3 方案 2：扩展（Query Expansion）— 始终开启
...
**适用 Worker：** Doc Worker 和 Code Worker 收益最大；SQL Worker 有标准化就够，扩展容易引入不相关术语干扰 Schema 检索。

**风险：** 扩展过度会引入噪声。对策：限制扩展词数量 ≤ 5，且只在原始 query ≤ 30 字时触发。

### 4.4 方案 3：分解（Multi-Query Decomposition）— 条件触发
...
**与 Supervisor 路由的关系：** 分解在意图分类之后执行——分类先确定需要哪些源，分解再为每个源生成定制化检索词。

### 4.5 方案 4：HyDE（Hypothetical Document Embeddings）— 条件触发
...
3. 目标 Worker 是 Doc Worker（Code Worker 和 SQL Worker 不需要——Code Worker 用 grep 不依赖向量检索，SQL Worker 的 Schema RAG 不需要 HyDE）

**并行策略：** HyDE 生成的同时原始 query 的检索也先跑，HyDE 的结果作为补充检索而非替代。
...
### 4.6 方案 5：退一步改写（Step-Back Prompting）— Phase 3+
...
### 4.7 方案 6：上下文感知改写（Context-Aware Rewriting）— Phase 3+
...
| Phase | 引入方案 | 理由 |
|-------|---------|------|
| Phase 1（SQL Worker） | 标准化 + 扩展 | 标准化解决"用户表→users"映射；扩展补充查询词汇。Text-to-SQL 准确率的基础保障 |
