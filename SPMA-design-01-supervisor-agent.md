...
### Checkpointer 隔离 & Worker 输出格式

> Checkpointer 隔离和 WorkerOutput 格式是跨 Agent 的统一基础设施，详见 [基础设施与运维设计](SPMA-design-06-infrastructure.md)。

---

## 四、Agent Action Guard

> 工具白名单是跨 Agent 的统一基础设施，详见 [基础设施与运维设计](SPMA-design-06-infrastructure.md)。
...
**为什么权重不是连续的？** 10、5、2 三档对应三种截然不同的检索行为：

- **权重 10（精确匹配级）：** 对应的 Agent 可以**跳过语义搜索**，直接用元数据查询或 grep。延迟从秒级降到毫秒级。
- **权重 5（语义锚点级）：** 不能跳过语义搜索，但把搜索限定在一个功能域内，检索精度大幅提升。
- **权重 1-2（辅助过滤级）：** 只在元数据层面做减法，对检索策略本身没有影响。

**计算方式：** 遍历所有实体字段，非空（非 None、非空列表、非空字符串）则累加权重。总分与阈值比较：

- **≥ 10（rich）：** 至少有一个精确匹配级实体 → 可跳过语义搜索，直接元数据查询或 grep
- **5-9（partial）：** 有功能锚点但无精确匹配 → 混合检索（元数据过滤 + 语义搜索）
- **< 5（bare）：** 无任何有效检索锚点 → 纯语义搜索兜底或反问用户

**为什么不用 LLM 打分：** LLM 评判自己的输出质量是循环依赖——LLM 抽取了实体，然后问同一个 LLM"你觉得你抽得怎么样？"，它大概率会说"抽得很好"。完备度评估不涉及语义判断——"有没有抽到需求 ID"是纯规则问题，用代码就够了。
...
> Token 预算管理是跨 Agent 的统一基础设施，详见 [基础设施与运维设计](SPMA-design-06-infrastructure.md)。
...
> Agent 状态存储（进程内存/Redis/Postgres 三层）是跨 Agent 的统一基础设施，详见 [基础设施与运维设计](SPMA-design-06-infrastructure.md)。
...
> 回滚机制（feature flags）是跨 Agent 的统一基础设施，详见 [基础设施与运维设计](SPMA-design-06-infrastructure.md)。
