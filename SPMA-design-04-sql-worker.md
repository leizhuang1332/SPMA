...
SQL Agent 作为独立的 LangGraph 子图运行，节点与流转如下：
...
- **generate 节点：** LLM 根据 Schema RAG 检索结果 + 业务元数据生成 SQL
- **guard 节点：** SQLGlot 语法校验 + DDL/DML 拦截 + 表/列存在性验证 + 性能保护
- **execute 节点：** 在只读副本上执行 SQL，记录执行时间和数据快照
- **verify 节点：** 检查行数范围、统计异常、语义正确性；确定性条件优先（执行成功 AND 行数正常→自动收敛），不满足时 LLM 兜底
- **条件边：** guard 失败或 verify 不通过 → 携带错误/异常信息回到 generate（最多 5 轮），通过 → END
...
Agent 循环中每轮不仅反馈语法/执行错误，还反馈执行结果的统计异常。反馈信息的构造过程：

1. **语法/执行错误检测：** 如果上轮 SQL 执行抛出异常（语法错误、类型不匹配、权限不足等），将错误原文作为反馈信息
2. **空结果检测：** 返回 0 行时，反馈"可能原因：过滤条件过严、时间范围不对、表为空"
3. **超大量结果检测：** 返回超过 100,000 行时，反馈"可能缺少聚合或过滤条件"
4. **NULL 值异常检测：** 对结果集的每一列计算 NULL 比例，比例过高的列被标记
5. **分布异常检测：** 如果分组结果的分布与预期不符（如某个分组占比异常），反馈分布摘要

所有反馈信息拼接后作为下一轮 LLM SQL 生成的修正提示。
...
在执行前对生成的 SQL 做模式匹配，匹配到以下任一高风险特征时需要用户确认：

| 风险特征 | 匹配模式 | 风险说明 |
|---------|---------|---------|
| 涉及财务指标 | SQL 含聚合函数（SUM/AVG/COUNT）且列名涉及金额/营收/工资等 | 财务数据敏感，用户需确认口径 |
| 全表扫描 | COUNT(*) 或 SUM 后无有效 WHERE 条件 | 可能很慢，且用户可能只需要部分数据 |
| 跨多表 JOIN | 三个以上 JOIN 子句 | 逻辑复杂度高，容易出错 |
| 时间范围异常 | INTERVAL 含 year/month 级别跨度 | 数据量大，可能超出预期范围 |

确认时向用户展示：即将执行的 SQL 原文、涉及的表和字段、时间范围、预计返回行数和风险提示，提供"确认执行"和"修改查询"两个选项。
...
执行 SQL 后对查询结果做基本的统计质量扫描，生成 `QualityReport`。质量检测不阻塞返回——结果照样展示，但在结果下方附加质量标注。

扫描检查项：

1. **空结果检测：** 行数 = 0 时，标注可能原因（过滤条件过严、时间范围不对、表名选错）
2. **NULL 比例异常：** 对每列计算 NULL 值占比，比例 > 50% 的列被标记，提示"可能影响聚合结果"
3. **数值列异常值检测：** 对每个数值列计算基本统计量（max / P99 / P50），如果 max > P99 × 10 则标记极端值
4. **时间范围合理性：** 如果 SQL 涉及 `created_at` 或 `date` 列的过滤，检查查询覆盖的时间跨度是否 > 365 天

质量报告置信度公式：`confidence = 1.0 - (问题数 × 0.2)`——每个问题扣 20% 置信度。
